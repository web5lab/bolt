...
const PricingPlan = ({ name, price, description, features, isPopular, icon: Icon, gradient }) => {
  ...
  return (
    <div
      className={`relative flex flex-col h-full group transition-all duration-500 transform hover:scale-105 ${isPopular
    ...
    >
      ...
      <div className="p-6 border-b border-slate-700/30 relative z-10">
        ...
        <div className="mb-6">
          <div className="flex items-baseline gap-1">
            <span className={`text-4xl font-bold transition-all duration-300 ${isPopular
        ...
        <button className={`w-full py-3 px-4 rounded-lg font-medium transition-all duration-300 transform hover:scale-105 shadow-lg relative overflow-hidden group/btn ${isPopular
        ...
        <ul className="space-y-3">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start group/item">
              <div className={`mr-3 mt-0.5 flex-shrink-0 transition-all duration-300 ${feature.included ? 'group-hover/item:scale-110' : ''
                }`}>
                {feature.included ? (
                  <div className="relative">
              ...
              <span className={`transition-colors duration-300 ${feature.included
      ...
      {/* Hover glow effect */}
      <div className={`absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none ${isPopular
